...
/* ============================ */
short 
print_results(t_gap_instance *t_gap_instance, t_gap_solution *t_gap_solution)
{
char file_name[80] ;
FILE *res_file ;
int valeur ;
...
strcpy(file_name,t_gap_instance->name) ;
...
if (res_file==NULL)
	...
	{
	fprintf(res_file, "============================\n");
	fprintf(res_file, "Nombre d'agents  %d\n", t_gap_instance->agent_count);
	fprintf(res_file, "Nombre de tâches %d\n", t_gap_instance->job_count);
	/* écriture des affectations (x(it)) */
	for (agt=0;agt < t_gap_instance->agent_count;agt++)
		{
		/* pour chaque agent agt */
		fprintf(res_file, "Agent n°%d\n", agt+1);
		/* capacité initiale et résiduelle */
		for (job=0;job < t_gap_instance->job_count;job++)
			{
			if (t_gap_solution->assignment[agt][job] == 1)
				{
				ass_num++ ;
				} ;
			} ;
		fprintf(res_file, "\tcapacité initiale: %3d\t résiduelle: %3d\tnb tâches: %3d\n", 
			t_gap_instance->capacity[agt], t_gap_solution->capacity_left[agt], ass_num);
